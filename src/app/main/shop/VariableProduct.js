...
import {
  ...
  selectProductByVariation,
} from "app/store/homeSlice";
import { TopSellers } from "../home/top-sellers/TopSellers";

export const VariableProduct = () => {
  const singleProduct = useSelector(selectProductByVariation);
  const dispatch = useDispatch();
  const quantity = useSelector(selectQuantity);
  const cartSubtotal = useSelector(selectCartSubtotal);
  const cart = useSelector(selectCart);
  const { id, variations } = useParams();
  const [alignment, setAlignment] = useState(
    singleProduct?.variationData?.[0]._id
  );

  useEffect(() => {
    dispatch(
      getProductByVariation({ _id: id, variationIds: variations.split(",") })
    ).then(() => window.scrollTo(0, 0));
  }, [variations]);

  useEffect(() => {
    setAlignment(singleProduct?.variationData?.[0]._id);
  }, [singleProduct]);
      ...
      <div className="md:flex w-full md:space-x-32">
          ...
          <img
            src={
              singleProduct.type === "Variable"
                ? singleProduct?.variations?.find(
                    (variation) => variation.variationIds[0] === alignment
                  )?.images?.[0]
        ...
        <div className="md:w-1/2">
          <Typography className="single-product-title">
            {singleProduct?.title}
            <Typography>
              SKU: {singleProduct?.productSku} {alignment}
              <br />
              Categories: {singleProduct?.category?.[0]?.title},{" "}
              {singleProduct?.brand?.[0]?.title}
            </Typography>
          </Typography>
          ...
          {singleProduct?.type !== "Simple" && (
            <>
              ...
              <div className="flex items-center space-x-4 my-16">
                ...
                >
                  {singleProduct?.variationData?.map((variation, index) => (
          ...
          <div className="flex w-full items-center space-x-16">
            ...
            <Button
              ...
              disabled={quantity < 1}
              onClick={() => {
                dispatch(updateCartSubtotal(quantity * singleProduct.price));
                dispatch(
                  updateCart({
                    id: id,
                    quantity: quantity,
                    price: singleProduct?.variations?.find(
                      (variation) => variation.variationIds[0] === alignment
                    )?.price,
                    title: singleProduct?.title,
                    image: singleProduct?.variations?.find(
                      (variation) => variation.variationIds[0] === alignment
                    )?.images?.[0],
                    subtotal:
                      quantity *
                    ...
                    product: id,
                    sellPrice: singleProduct?.variations?.find(
                      (variation) => variation.variationIds[0] === alignment
                    )?.price,
                    selectedVariation:
